...
import { Camera, Sparkles, BookOpen, Heart, PlayCircle, ChevronLeft, ChevronRight, X, Film } from 'lucide-react';
import { useState, useEffect, useCallback } from 'react';
import { cn } from '@/lib/utils';
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from '@/components/ui/carousel';
import { Dialog, DialogContent } from '@/components/ui/dialog';

import { GalleryImage, HomeContent, Service, AboutContent } from './types';
...
interface HomePageProps {
  initialData: {
    heroImagesData: GalleryImage[];
    portfolioImagesData: GalleryImage[];
    aboutDataRaw: AboutContent[];
    servicesDataRaw: Service[];
  };
}

export default function HomePage({ initialData }: HomePageProps) {
  const [content, setContent] = useState<HomeContent>(() => ({
    heroImages: initialData.heroImagesData || [],
    portfolioImages: initialData.portfolioImagesData || [],
  }));
  ...
  const [aboutContent, setAboutContent] = useState<string[]>(() => {
    ...
    if (Array.isArray(initialData.aboutDataRaw) && initialData.aboutDataRaw.length > 0) {
      const aboutObj = initialData.aboutDataRaw[0];
      if (Array.isArray(aboutObj.about)) {
        return aboutObj.about;
      }
      if (typeof aboutObj.about === 'string') {
        return [aboutObj.about];
      }
    }
  ...
  const [services, setServices] = useState<Service[]>(() => {
    ...
    if (Array.isArray(initialData.servicesDataRaw) && initialData.servicesDataRaw.length > 0) {
      ...
      const validServices = initialData.servicesDataRaw.filter(isService);
